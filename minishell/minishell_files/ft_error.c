...
int	ft_error(int nb, char *str)
{
	...
	if (nb == 1)
		...
		msg = ft_multi_strjoin("%s '%s'", "minishell: parse error, "
			"unexpected token", str);
	else if (nb == 3)
		msg = ft_multi_strjoin("%s %s", "minishell: execute error, "
			"command not found:", str);
	else if (nb == 4)
		msg = ft_multi_strjoin("%s %s %s", "minishell:", str , "No such file or directory");
	else if (nb == 5)
		msg = ft_multi_strjoin("%s %s %s", "minishell:", str , "ambiguous redirect");
	else
